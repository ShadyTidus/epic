...
function onLoad(saved_data)
    ...
    for _, guid in ipairs(CARD_ZONE_GUIDS) do
        ...
        if zone then table.insert(cardZones, zone) end
    end

    lZone = {}
    local legendZoneObject = getObjectFromGUID(LEG_ZONE)
    if legendZoneObject then table.insert(lZone, legendZoneObject) end
    ...
    -- Create World Action Buttons
    local anchor = spawnObject({type="Block", position={-49.02, 3.52, -14.80}, rotation={0,0,0}, scale={0.1, 0.1, 0.1}})
    anchor.setLock(true)
    anchor.interactable = false
    anchor.hide("All")
    anchor.createButton({label="Get Familiar", click_function="getFamiliars", function_owner=self, position={0, 0, 0}, height=200, width=800, font_size=100})
    anchor.createButton({label="Get Legend", click_function="legends", function_owner=self, position={0, 0, -2.73}, height=200, width=800, font_size=100})
    anchor.createButton({label="Get Ability", click_function="getAbility", function_owner=self, position={0, 0, -5.46}, height=200, width=800, font_size=100})
    anchor.createButton({label="Get DW Tokens", click_function="dWToken", function_owner=self, position={0, 0, -8.19}, height=200, width=800, font_size=100})
